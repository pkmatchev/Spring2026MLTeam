...
class ConvLayer:
    ...
    def forward(self, x):
        ...
        #2. Compute output dimensions: 
        N, C, H, W = x.shape
        H_out = (H + 2 * p - self.kernel_size) // self.stride + 1
        W_out = (W + 2 * p - self.kernel_size) // self.stride + 1
        ...
        for f in range(self.num_filters):
            for i in range(H):
                for j in range(W):
                    patch = x_padded[:, :, i * self.stride : i * self.stide + K, j * self.stride : j * self.stide + K]
                    out[:, f, i, j] = np.sum(patch * self.W[f], axis=(1,2,3)) + self.b[f]
        
        #Cache x_padded and x.shape for the backwardpass 
    ...
    def backward(self, dout):
        ...
        """
        # *** TODO ***
        #1. Allocate space for gradient matrix
        ...
        for f in range(self.num_filters):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    dout_ij = dout[: , f, i, j]
                    dW[f] += np.sum(dout_ij[:, None, None, None] * patch)
                    dx_padded[:, :, i*s:i*s+K, j*s:j*s+K] += dout_ij[:,None,None,None] * W[f]
                    
        #Strip padding from dx_padded to get dx:
        p = self.padding
...
class CNN:
    ...
    def backward(self, grad):
        ...
        *** TODO ***
        ...
        # 3. Un-flatten: (N, 400) -> (N, 16, H_pool2, W_pool2)
        grad = grad.reshape(self.flat_shape)

        # 4. Pool 2 -> ReLU 2 -> Conv 2
        grad = self.pool2.backward(grad)
        grad = relu_backward(grad, self.relu2_input)
        grad = self.conv2.backward(grad)

        # 5. Pool 1 -> ReLU 1 -> Conv 1
