    ...
    def forward(self, x):
        ...
        for i in range(H_out):
            for j in range(W_out):
                ...
                # Sum over channels, height, and width of the patch/filter
    ...
    def backward(self, dout):
        ...
        for i in range(H_out):
            for j in range(W_out):
                ...
                for f in range(F):
                    # dW calculation
                    dW[f] += np.sum(dout[:, f, i, j][:, None, None, None] * patch, axis=0)
                    # dx calculation
                    dx_padded[:, :, i*S:i*S+K, j*S:j*S+K] += dout[:, f, i, j][:, None, None, None] * self.W[f]

        self.dW, self.db = dW, db
        
        if P > 0:
            return dx_padded[:, :, P:dx_padded.shape[2]-P, P:dx_padded.shape[3]-P]  
        else:
            return dx_padded
    ...
    def __init__(self):
        ...
        *** TODO ***
        ...
        Architecture reminder:
            ...
            fc2    — FCLayer: 128 → 10
        """
        self.conv1 = ConvLayer(in_channels=1, num_filters=8, kernel_size=3)   # *** TODO ***
        self.pool1 = MaxPoolLayer(pool_size=2, stride=2)   # *** TODO ***
        self.conv2 = ConvLayer(in_channels=8, num_filters=16, kernel_size=3)  # *** TODO ***
        self.pool2 = MaxPoolLayer(pool_size=2, stride=2)   # *** TODO ***
        self.fc1   = FCLayer(in_features=400, out_features=128)   # *** TODO ***
        self.fc2   = FCLayer(in_features=128, out_features=10)   # *** TODO ***
